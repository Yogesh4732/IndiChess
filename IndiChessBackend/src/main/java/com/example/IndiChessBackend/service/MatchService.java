...
public class MatchService {
    ...
    // Store waiting players and their match IDs for STANDARD (existing queue)
    ...
    // Separate queues for RAPID time control
    private static final Map<String, Long> waitingRapidPlayers = new ConcurrentHashMap<>();
    private static final Map<Long, String[]> matchRapidPlayers = new ConcurrentHashMap<>();
        ...
        synchronized(this) {
            ...
            for (String waitingPlayer : waitingPlayers.keySet()) {
                if (!waitingPlayer.equals(userName)) {
                    // Found opponent
                    User player1 = userRepo.getUserByUsername(waitingPlayer); // The waiting user is white
                    User player2 = userRepo.getUserByUsername(userName);      // The joining user is black
    ...
    // Create a RAPID (10-minute) match using a separate rapid queue
    public Optional<Long> createRapidMatch(HttpServletRequest request) {
        String tk = getJwtFromCookie(request);
        String userName = jwtService.extractUsername(tk);
        ...
        System.out.println("User " + userName + " requesting RAPID match");

        synchronized(this) {
            // Check if there's already a waiting rapid player
            for (String waitingPlayer : waitingRapidPlayers.keySet()) {
                if (!waitingPlayer.equals(userName)) {
                    // Found opponent
                    User player1 = userRepo.getUserByUsername(waitingPlayer); // waiting user is white
                    User player2 = userRepo.getUserByUsername(userName);      // joining user is black

                    if (player1 != null && player2 != null) {
                        Match newMatch = new Match(player1, player2, IN_PROGRESS, 1);
                        newMatch.setGameType(GameType.RAPID);
                        newMatch = matchRepo.save(newMatch);
                        Long matchId = newMatch.getId();

                        matchRapidPlayers.put(matchId, new String[]{waitingPlayer, userName});
                        waitingRapidPlayers.remove(waitingPlayer);

                        System.out.println("RAPID match created: " + matchId);
            ...
            // No opponent found, add to RAPID waiting queue
            waitingRapidPlayers.put(userName, -1L);
            System.out.println("User " + userName + " added to RAPID waiting queue");
    ...
    // Method for Player1 to check if RAPID match was created
    public Optional<Long> checkRapidMatch(HttpServletRequest request) {
        String tk = getJwtFromCookie(request);
        String userName = jwtService.extractUsername(tk);
        ...
        synchronized(this) {
            // Check if user is in rapid waitingPlayers
            if (waitingRapidPlayers.containsKey(userName)) {
                return Optional.of(-1L);
            }

            // Check if user has a rapid match
            for (Map.Entry<Long, String[]> entry : matchRapidPlayers.entrySet()) {
                String[] players = entry.getValue();
                if (players[0].equals(userName) || players[1].equals(userName)) {
                    Long matchId = entry.getKey();
                    matchRapidPlayers.remove(matchId);
                    waitingRapidPlayers.remove(players[0]);
                    waitingRapidPlayers.remove(players[1]);
                    System.out.println("Returning RAPID match " + matchId + " to " + userName);
                    return Optional.of(matchId);
                }
            }
        }
    ...
    // Method to cancel RAPID waiting
    public boolean cancelRapidWaiting(HttpServletRequest request) {
        String tk = getJwtFromCookie(request);
        String userName = jwtService.extractUsername(tk);
        ...
        synchronized(this) {
            boolean removed = waitingRapidPlayers.remove(userName) != null;
            if (removed) {
                System.out.println("User " + userName + " cancelled RAPID waiting");
            }
            return removed;
        }
    }
