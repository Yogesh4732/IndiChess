...
public class GameService {
    ...
    public GameDTO getGameDetails(Long matchId, HttpServletRequest request) {
        ...
        if (request != null) {
            ...
            System.out.println("[GameService] Extracted username: " + username);
        }

        Optional<Match> matchOpt = matchRepo.findById(matchId);
        if (matchOpt.isEmpty()) {
            System.out.println("[GameService] Game not found for matchId: " + matchId);
        ...
        // Only set player-specific fields if username is available
        if (username != null) {
            // Determine player color
            String playerColor = determinePlayerColor(match, username);
            System.out.println("[GameService] Returning playerColor: " + playerColor + " for username: " + username);
            // Determine if it's this player's turn
            boolean isMyTurn = determineMyTurn(match, username);
            System.out.println("[GameService] isMyTurn: " + isMyTurn + " for username: " + username);
            gameDTO.setPlayerColor(playerColor);
            gameDTO.setMyTurn(isMyTurn);
        } else {
            gameDTO.setPlayerColor(null);
            gameDTO.setMyTurn(false);
        }
    ...
    public MoveDTO processMove(Long matchId, MoveRequest moveRequest, Principal principal) {
        ...
        GameDTO updatedGame = getGameDetails(matchId, null); // null is fine, just for state broadcast
        messagingTemplate.convertAndSend("/topic/game/" + matchId, updatedGame);
    ...
    private void updateMatchInDatabase(Long matchId, MoveRequest moveRequest) {
        try {
            ...
            if (matchOpt.isPresent()) {
                Match match = matchOpt.get();
                // Compute next ply purely from moves table so Match is not used as a move counter
                long existingMoves = moveRepo.countByMatch_Id(matchId);
                int newPly = (int) existingMoves + 1;
                ...
                try {
                    ...
                    moveEntity.setPly(newPly);
                    moveEntity.setMoveNumber((newPly + 1) / 2); // 1,2 -> 1; 3,4 -> 2; etc.

                    // Color based on playerColor string from request
                    String colorStr = moveRequest.getPlayerColor();
                    if ("white".equalsIgnoreCase(colorStr)) {
                        moveEntity.setColor(PieceColor.WHITE);
                    } else if ("black".equalsIgnoreCase(colorStr)) {
                        moveEntity.setColor(PieceColor.BLACK);
                    }
                    ...
                    moveRepo.save(moveEntity);
                    System.out.println("💾 Saved move entity with id: " + moveEntity.getId());
                } catch (Exception ex) {
                    System.err.println("⚠️ Failed to persist Move entity: " + ex.getMessage());
                }
            } else {
    ...
    public void handleResignation(Long matchId, String username) {
        ...
        if (gameState != null) {
            ...
            // Persist match result: the opponent wins
            try {
                Optional<Match> matchOpt = matchRepo.findById(matchId);
                if (matchOpt.isPresent()) {
                    Match match = matchOpt.get();
                    String player1Username = match.getPlayer1().getUsername();
                    String player2Username = match.getPlayer2().getUsername();
                    ...
                    System.out.println("🏁 Match " + matchId + " updated with resignation result: " + match.getStatus());
                }
            } catch (Exception e) {
    ...
    public void markDraw(Long matchId) {
        ...
        // Persist DRAW status in matches table
        try {
            Optional<Match> matchOpt = matchRepo.findById(matchId);
            if (matchOpt.isPresent()) {
                Match match = matchOpt.get();
                ...
                matchRepo.save(match);
                System.out.println("🏁 Match " + matchId + " marked as DRAW");
            }
        } catch (Exception e) {
            System.err.println("⚠️ Failed to update match status to DRAW: " + e.getMessage());
        }
    }
    ...
    private String extractToken(HttpServletRequest request) {
        ...
        try {
            ...
            System.err.println("[GameService] Warning: request.getHeader threw exception: " + e.getMessage());
        }
        ...
        try {
            ...
            // Defensive: log and continue
            System.err.println("[GameService] Warning: request.getCookies threw exception: " + e.getMessage());
        }
        if (cookies != null) {
