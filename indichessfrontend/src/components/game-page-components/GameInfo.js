    ...
    try {
      ...
      if (response.ok) {
        ...
        if (!contentType.includes('application/json')) {
          ...
          console.error('Unexpected response when creating game:', text);
          setIsSearching(false);
        ...
        // Non-2xx response; likely unauthorized when user is not logged in
        const text = await response.text().catch(() => '');
        console.error('Failed to create match. Status:', response.status, 'Body:', text);
        setIsSearching(false);
