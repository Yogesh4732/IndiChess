...
const Board = ({ 
  ...
  // Use isMyTurn directly for turn logic
...
useEffect(() => {
    if (opponentMove && opponentMove !== lastOpponentMove) {
      ...
      // Error move check: skip if moveNotation starts with 'ERROR' or all fields are null
      if (
        (typeof opponentMove.moveNotation === 'string' && opponentMove.moveNotation.startsWith('ERROR')) ||
        (
          opponentMove.fromRow == null && opponentMove.fromCol == null &&
          opponentMove.toRow == null && opponentMove.toCol == null &&
          opponentMove.piece == null && opponentMove.board == null
        )
      ) {
        console.warn("⚠️ Skipping invalid/error move from server:", opponentMove.moveNotation || opponentMove);
        setLastOpponentMove(opponentMove); // Prevent reprocessing
        return;
      }
...
const createMoveNotation = (from, to, piece, capturedPiece, castled, board) => {
  ...
  const canCastleGeneral = () => {
    ...
    if ((isWhite && isKingInCheckHook[0]) || (!isWhite && isKingInCheckHook[1]) ||
        (isWhite && kingsMoved[0]) || (!isWhite && kingsMoved[1])) {
      return false;
    }
    return true;
  };
  ...
  const handleSquareClick = (row, col) => {
    ...
    // Only allow moving your own color's pieces
    if (!piece || (playerColor === 'white' && !isUpperCase(piece)) || (playerColor === 'black' && isUpperCase(piece))) return;
  ...
  const handleDragStart = (e, row, col) => {
    ...
    const piece = board[row][col];
    // Only allow moving your own color's pieces
    if (!piece || (playerColor === 'white' && !isUpperCase(piece)) || (playerColor === 'black' && isUpperCase(piece))) {
      e.preventDefault();
      return;
    }
    setIsSquareSelected(true);
  ...
  const isKingInCheck = () => {
    for(let i = 0; i<8; i++){
      for(let j = 0; j<8; j++){
        ...
        // Use isMyTurn and playerColor for logic
        if( (playerColor === 'white' && isUpperCase(piece)) || (playerColor === 'black' && !isUpperCase(piece)) ){
          const kingRow = playerColor === 'white' ? blackKingCoordinates[0] : whiteKingCoordinates[0];
          const kingCol = playerColor === 'white' ? blackKingCoordinates[1] : whiteKingCoordinates[1];
  ...
  const getMovesOfPlayer = () => {
    ...
    for(let i = 0; i<8; i++){
      for(let j = 0; j<8; j++){
        ...
        if(piece && ((isWhitePiece && isWhitePlayer) || (!isWhitePiece && !isWhitePlayer)) ){
